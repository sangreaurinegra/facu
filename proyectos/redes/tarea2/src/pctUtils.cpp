...
int crearPaqueteKAL(char* datagrama, int numKAL){
	...
	char buf[10];
	sprintf(buf,"KAL-%d", numKAL);
	return crearPaquete(0, numKAL, 0, buf, 10 ,datagrama);
}
...
int recibirPaquete(char * datosPaquetes, size_t & lenDatos, unsigned char & flag) {
	//log("pcutils.cpp --- recibirPaquete",LOGGER_DEBUG);
			...
			while ( recv_result < 0) {
				...
				if (selectResult == 0) {
					//log("pcutils.cpp - recibirPaquete ERROR recibirPaqueteControl: select Time out",LOGGER_DEBUG);
					//return -1;
				}
				if (selectResult == -1) {
					//log("pcutils.cpp - recibirPaquete ERROR recibirPaqueteControl: Error al hacer select",LOGGER_DEBUG);
					return -1;
				}
				...
				if (recv_result < 0 ) {
					//log("pcutils.cpp - recibirPaquete ERROR recibirPaqueteControl: Error al hacer recvfrom",LOGGER_DEBUG);
					return -1;
				}
				...
				if (datagrama != NULL) {
						...
						if (ip->daddr == globalIPOrigen.s_addr && pct->destPort == globalPuertoOrigen){
							...
							if (flag == DATA_FLAG) { // Es un paquete de datos devuelvo los datos, cambio el tamanio de los datos
								...
								lenDatos = recv_result - (sizeof(struct iphdr)+sizeof(struct pct_header));
								memcpy (datosPaquetes,datos,lenDatos);
...
int enviarPaqueteControlDirecto (unsigned char flag) {
	...
	log ("pcutils.cpp - ENTRE A enviarPaqueteControlDirecto",LOGGER_DEBUG);
	...
	char buf[10];
	sprintf(buf,"MIO-%d", 0);
	crearPaquete(0, 0, flag, buf, 10 ,datagrama);

	int res = enviarPaquete(datagrama,0);
	log ("pcutils.cpp - enviarPaqueteControlDirecto > Retorno de enviarPaquete: "+intToStr(res),LOGGER_DEBUG);
...
int recibirPaqueteControl (unsigned char flag) {
	...
	while ( recv_result < 0) {
		...
		if (selectResult == 0) {
			//log("pcutils.cpp - ERROR recibirPaqueteControl: select Time out",LOGGER_DEBUG);
			return -1;
		}
		if (selectResult == -1) {
			//log("pcutils.cpp - ERROR recibirPaqueteControl: Error al hacer select",LOGGER_DEBUG);
			return -1;
		}
		...
		recv_result = recvfrom(globalPctSocket, datagrama, tamanioDatagrama, MSG_WAITALL,(struct sockaddr *)&direccionSocketOrigen, (socklen_t *)&socketSize) ;
		if (recv_result < 0 ) {
			//log("pcutils.cpp - ERROR recibirPaqueteControl: Error al hacer recvfrom",LOGGER_DEBUG);
			return -1;
		}
