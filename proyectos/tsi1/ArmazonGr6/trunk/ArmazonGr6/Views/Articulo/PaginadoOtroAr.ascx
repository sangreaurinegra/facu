...
<%} else { %>
    ...
    <%if (Model.getListaOtroAr().Count()>0) { %>

        <% if (Model.getListaOtroAr().hasPreviousPage)
           { %>
                <%= Ajax.ActionLink("<<", "MostrarPaginaOtroAr", "Articulo", new { pagActual = Model.getPagActOtroAr() - 1, texto = Model.getTexto() }, new AjaxOptions { UpdateTargetId = "otroArId" })%>
        <%} %>
        
        <%= Html.Encode(Model.getPagActOtroAr())%>
        
        <% if (Model.getListaOtroAr().hasNextPage)
           { %>
                <%= Ajax.ActionLink(">>", "MostrarPaginaOtroAr", "Articulo", new { pagActual = Model.getPagActOtroAr() + 1, texto = Model.getTexto() }, new AjaxOptions { UpdateTargetId = "otroArId" })%>
        <%} %>
        ...
        <% foreach (var item in Model.getListaOtroAr()) { %>
            ...
            <tr>
                ...
                if (Request.IsAuthenticated && !esAdmin) {  %>
                    <td style="border-style:none;">
                        <%= Html.DropDownList("list_OAr"+item.id, ViewData["list"] as SelectList)%>    
                    </td>
                    <td style="border-style:none;">
                        <input type="button"  value="Añadir al carrito" onclick="agregarAlCarrito(<%=item.id%>, document.getElementById('list_OAr<%= item.id %>').options[document.getElementById('list_OAr<%= item.id %>').selectedIndex].value)" />          
                    </td>
                <% } %>               
            </tr>
    ...
    <%} else { %>
        La lista devuelta por el Otro Armazon es vacía.
    <%} %>
<% } %>